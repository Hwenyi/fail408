...
Q:二次型一定可以化成标准型和规范形吗？
A:定理 1 任何二次型 $f\left( \mathbf{x} \right) = {\mathbf{x}}^{T}\mathbf{A}\mathbf{x}$ 均可通过配方法 (作可逆线性变换 $\mathbf{x} = \mathbf{C}\mathbf{y}$ ) 化成标准形及规范形, 用矩阵语言表述: 任何[[实对称矩阵]] $\mathbf{A}$ ,必存在[[逆矩阵|可逆矩阵]] $\mathbf{C}$ ,使得 ${\mathbf{C}}^{T}\mathbf{A}\mathbf{C} = \mathbf{\Lambda}$ ,其中
...
A:定理 2 任何二次型 $f\left( \mathbf{x} \right) = {\mathbf{x}}^{T}\mathbf{A}\mathbf{x}$ 也可以通过[[正交变换]] $\mathbf{x} = \mathbf{Q}\mathbf{y}$ 化成[[标准形]],用矩阵语言表述: 任何[[实对称矩阵]] $\mathbf{A}$ ,一定存在[[正交矩阵]] $\mathbf{Q}$ ,使得
$$
