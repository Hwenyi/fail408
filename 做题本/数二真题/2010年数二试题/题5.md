...
### 题目
> [!question]+
> (5) 设函数 $z = z\left( {x,y}\right)$ 由方程 $F\left( {\frac{y}{x},\frac{z}{x}}\right)  = 0$ 确定,其中 $F$ 为可微函数,且 ${F}_{2}^{\prime } \neq  0$ ,则$x\frac{\partial z}{\partial x} + y\frac{\partial z}{\partial y} =$ 
> (A) $x$ . 
> (B) $z$ . 
> (C) $- x$ . 
> (D) $- z$ .
...
### 解
> [!done]-
> 解 对方程 $F\left( {\frac{y}{x},\frac{z}{x}}\right)  = 0$ 两端同时关于 $x,y$ 求偏导数,可得
> 
> $$
> - \frac{y}{{x}^{2}}{F}_{1}^{\prime } + \left( {-\frac{z}{{x}^{2}} + \frac{1}{x} \cdot  \frac{\partial z}{\partial x}}\right) {F}_{2}^{\prime } = 0,\;\frac{{F}_{1}^{\prime }}{x} + \frac{{F}_{2}^{\prime }}{x} \cdot  \frac{\partial z}{\partial y} = 0.
> $$
> 
> 由上面两个方程解得,
> 
