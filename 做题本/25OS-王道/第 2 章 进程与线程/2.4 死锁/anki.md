...
### 循环等待条件
Q: 什么是循环等待条件？
A: 存在一种进程资源的循环等待链, 链中每个进程已获得的资源同时被链中下一个进程所请求。即存在一个处于等待态的进程集合 $\left\{ {{P}_{1},{P}_{2},\cdots ,{P}_{n}}\right\}$ ,其中 ${P}_{i}$ 等待的资源被 ${P}_{i + 1}\left( {i = 0,1,\cdots, n - 1}\right)$ 占有, ${P}_{n}$ 等待的资源被 ${P}_{0}$ 占有,如图 2.13 所示。
...
Q: 什么是安全状态？
A: 所谓安全状态,是指系统能按某种进程推进顺序 $\left( {{P}_{1},{P}_{2},\cdots ,{P}_{n}}\right)$ 为每个进程 ${P}_{i}$ 分配其所需的资源,直至满足每个进程对资源的最大需求,使每个进程都可顺利完成。此时称 ${P}_{1},{P}_{2},\cdots ,{P}_{n}$ 为安全序列 (可能有多个)。若系统无法找到一个安全序列, 则称系统处于不安全状态。
...
1. 可利用资源向量 Available: 含有 $m$ 个元素的数组,其中每个元素代表一类可用的资源数目。Available $\left\lbrack j\right\rbrack = K$ 表示此时系统中有 $K$ 个 ${R}_{j}$ 类资源可用。
2. 最大需求矩阵 Max: $n \times m$ 矩阵,定义系统中 $n$ 个进程中的每个进程对 $m$ 类资源的最大需求。 $\operatorname{Max}\left\lbrack {i, j}\right\rbrack = K$ 表示进程 ${P}_{i}$ 需要 ${R}_{j}$ 类资源的最大数目为 $K$ 。
3. 分配矩阵 Allocation: $n \times m$ 矩阵,定义系统中每类资源当前已分配给每个进程的资源数。 Allocation $\left\lbrack {i, j}\right\rbrack = K$ 表示进程 ${P}_{i}$ 当前已分得 ${R}_{j}$ 类资源的数目为 $K$ 。
4. 需求矩阵 Need: $n \times m$ 矩阵,表示每个进程接下来最多还需要多少资源。Need $\left\lbrack {i, j}\right\rbrack = K$ 表示进程 ${P}_{i}$ 还需要 ${R}_{j}$ 类资源的数目为 $K$ 。
...
### 银行家算法描述
Q: 银行家算法的步骤是什么？
A: 设 Request ${}_{i}$ 是进程 ${P}_{i}$ 的请求向量, $\operatorname{Request},\left\lbrack j\right\rbrack = K$ 表示进程 ${P}_{i}$ 需要 $j$ 类资源 $K$ 个。当 ${P}_{i}$ 发出资源请求后, 系统按下述步骤进行检查:
① 若 ${\operatorname{Request}}_{i}\left\lbrack j\right\rbrack \leq \operatorname{Need}\left\lbrack {i, j}\right\rbrack$ ,则转向步骤②; 否则认为出错,因为它所需要的资源数已超过它所宣布的最大值。
② 若 Request $\left\lbrack j\right\rbrack \leq$ Available $\left\lbrack j\right\rbrack$ ,则转向步骤③; 否则,表示尚无足够资源, ${P}_{i}$ 必须等待。
③ 系统试探着将资源分配给进程 ${P}_{i}$ ,并修改下面数据结构中的数值:
$$
\text{ Available } = \text{ Available } - \text{ Request }{}_{i}\text{; }
$$
$$
\text{Allocation}\left\lbrack {i, j}\right\rbrack = \text{Allocation}\left\lbrack {i, j}\right\rbrack + {\text{Request}}_{i}\left\lbrack j\right\rbrack \text{;}
$$
$$
\operatorname{Need}\left\lbrack {i, j}\right\rbrack = \operatorname{Need}\left\lbrack {i, j}\right\rbrack - {\operatorname{Request}}_{i}\left\lbrack j\right\rbrack
$$
...
③ 进程 ${P}_{i}$ 进入安全序列后,可顺利执行,直至完成,并释放分配给它的资源,所以应执行 Work $=$ Work + Allocation $\left\lbrack i\right\rbrack$ ,其中 Allocation $\left\lbrack i\right\rbrack$ 是 Allocation 矩阵中对应的行,返回步骤②。
