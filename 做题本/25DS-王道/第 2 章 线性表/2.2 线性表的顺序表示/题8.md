...
> [!done]+
> C
> 对于 $\mathrm{I}$ ,解析略; 对于 $\mathrm{{II}}$ ,在最后位置插入新结点不需要移动元素,时间复杂度为 $O\left( 1\right)$ ; 对于 III,被删结点后的结点需要依次前移,时间复杂度为 $O\left( n\right)$ ; 对于 IV,需要后移 $n - i$ 个结点, 时间复杂度为 $O\left( n\right)$ 。