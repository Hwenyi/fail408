...
## 题7
### 题目
【2015】用单链表保存 $m$ 个整数，结点的结构为$\begin{array}{|c|c|} \hline \text{data} & \text{link}\\\hline \end{array}$，且 $\left| \text{data} \right|\le n$ （ $n$ 为正整数）。现要求设计一个时间复杂度尽可能高效的算法，对于链表中 $\text{data}$ 的绝对值相等的结点，仅保留第一次出现的结点而删除其余绝对值相等的结点。例如，若给定的单链表 $\text{head}$ 如下：
...
```
时间复杂度来自于遍历这个链表的长度:时间复杂度为 $O\left( m\right)$ ,空间复杂度为 $O\left( n\right)$
### 解
