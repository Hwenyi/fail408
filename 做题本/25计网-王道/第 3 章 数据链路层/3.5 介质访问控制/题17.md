...
### 分析
A:对于 ${100}\mathrm{{Mb}}/\mathrm{s}$ 的以太网,争用期与 ${10}\mathrm{{Mb}}/\mathrm{s}$ 的以太网相同,仍然是 ${64}\mathrm{\;B}$ 的发送时间,即 ${512}\mathrm{\;b}$:${100}\mathrm{{Mb}}/\mathrm{s} = {5.12\mu }\mathrm{s}$ 。根据 CSMA/CD 协议的退避算法,第 $k$ 次重传需要退避的时间为: 从整数集合  $\left\{  {0,1,\cdots ,{2}^{k} - 1}\right\}$ 中随机取出一个数 $r$ ,退避时间就是 $r$ 倍的争用期。当重传次数大于 10 时, $k$ 不再 增大,而一直等于 10 。本题中 $k = 2$ ,问的是最大退避时间,所以取 $r = 3$ ,最大退避时间为 ${15.36\mu }\mathrm{s}$
